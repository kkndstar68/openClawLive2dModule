...
def fetch_kline(code, days=60):
    ...
    eastmoney_url = f"http://push2his.eastmoney.com/api/qt/stock/kline/get?secid={code.replace('sh','1.').replace('sz','0.')}&fields1=f1,f2,f3,f4,f5,f6,f7,f8,f9,f10,f11,f12,f13&fields2=f51,f52,f53,f54,f55,f56,f57,f58,f59,f60,f61&klt=101&fqt=1&end=20500101&lmt={days}"
    
    gtimg_code = code.replace('sh', '')
    gtimg_url = f"http://web.ifzq.gtimg.cn/appstock/app/fkline/get?param={code},day,,,{days},qfq"
    
    headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'}
    
    for attempt in range(2):
        try:
            resp = requests.get(eastmoney_url, timeout=15, headers=headers)
            data = resp.json()
            kline_data = []
    ...
    for attempt in range(2):
        try:
            resp = requests.get(gtimg_url, timeout=15, headers=headers)
            data = resp.json()
            kline_data = []
            if data.get('data', {}).get(code, {}).get('day'):
                for item in data['data'][code]['day'][-days:]:
                    if len(item) >= 6:
                        kline_data.append({
                            'date': item[0],
                            'open': float(item[1]),
    ...
    print(f"[ERR] 获取K线失败 (双API都失败): {code}")
    return []
