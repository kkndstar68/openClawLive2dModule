...
class StockAnalyzer:
    ...
    def _calculate_ev(score: float, turnover: float) -> float:
        ...
        """
        # 评分贡献：60分以上有正期望
        score_contribution = (score - 50) / 10  # 60分 = 1, 70分 = 2, 等等
...
class ETFAnalyzer:
    ...
    ETF分析器
    权重: 技术面(100%)
    只关注流动性强的指数/行业ETF（科技类、黄金、稀土）
    """
    
    # 流动性最强的ETF代码
    LIQUID_ETFS = {
        '科技': ['512380', '512710', '512690', '159645'],  # 芯片、算力、光伏、机器人
        '新能源': ['159601', '159637'],  # 新能源、光伏
        '黄金': ['518880', '159934'],  # 黄金ETF
        '稀土': ['159800', '560000'],  # 稀土ETF
        '电力': ['515220', '570018'],  # 电力ETF
        '半导体': ['512480', '159648'],  # 半导体
    }
    ...
    def analyze(self, etf_data: Dict, kline_data: List[Dict]) -> Dict:
        """
        分析ETF
        etf_data: {'code': '512380', 'name': '芯片ETF', 'category': '科技'}
        """
        ...
        volume_scores = VolumeFactor.calculate({'量比': 1, '换手率': 2}, kline_data)
        rsi_scores = RSIFactor.calculate(kline_data)
        
        # 加权求和
        total_score = (
            (trend_scores.get('total', 0) / 35) * 40 +
            (volume_scores.get('total', 0) / 30) * 30 +
            (rsi_scores.get('total', 0) / 10) * 30
        )
        
        # 流动性检查（日均成交额 > 3000万）
        # 注意：K线数据中vol单位通常是"手"（1手=100股），需乘以100转换为股
        current_vol = kline_data[-1].get('vol', 0)
        current_price = kline_data[-1].get('close', 0)
        daily_turnover = current_vol * 100 * current_price / 100000000  # 转换成亿元
        ...
        return {
            ...
            'daily_turnover': round(daily_turnover, 2),  # 成交额（亿元）
            'score': min(total_score, 100),
            'is_liquid': is_liquid,
            'pass': total_score >= 50 and is_liquid,
            'trend': trend_scores.get('total', 0),
            'rsi': rsi_scores['rsi'],
            'signal': self._get_etf_signal(trend_scores, rsi_scores)
        }
...
class MarketContextAnalyzer:
    ...
    def analyze(index_data: List[Dict]) -> Dict:
        ...
        volatility = float(np.std(returns) * 100)  # 直接使用日收益率标准差百分比（通常0.5%-3%）
