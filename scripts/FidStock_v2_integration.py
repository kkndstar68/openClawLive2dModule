...
# 添加analyze目录到导入路径
analyze_path = os.path.join(os.path.dirname(__file__), 'analyze')
if analyze_path not in sys.path:
    sys.path.insert(0, analyze_path)

# 尝试导入新的多因子模块（从analyze目录）
try:
    ...
    print(f"[警告] 无法导入多因子模块，将使用降级方案: {e}")
...
def gen_active_codes():
    """生成活跃股票代码列表（调用FidStock真实实现）"""
    from FidStock import gen_active_codes as fid_gen_active_codes
    return fid_gen_active_codes()

def fetch_quotes(codes, timeout=12):
    """获取股票行情（调用FidStock真实实现）"""
    from FidStock import fetch_quotes as fid_fetch_quotes
    return fid_fetch_quotes(codes, timeout)

def fetch_kline(code, days=60):
    """获取股票K线数据（支持日K、周K、月K）"""
    from FidStock import fetch_kline as fid_fetch_kline
    result = fid_fetch_kline(code, days)
    if result:
        return result
    print(f"[Mock] 真实API失败，使用Mock数据: {code}")
    from mock_kline import generate_mock_kline
...
def scan_stocks(batch_size=100, max_stocks=500):
    ...
    原有的扫描函数（保留备用，降级方案）
    """
    from FidStock import scan_stocks as fid_scan_stocks
    return fid_scan_stocks(batch_size, max_stocks)
